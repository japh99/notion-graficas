...
def crear_app_interactiva(nombre_archivo, titulo, labels, datos, colores, mensaje):
    ...
    # Convertimos los datos de Python a texto JSON seguro para JavaScript
    ...
    <head>
        ...
        <style>
            ...
            button {{ background: #444; color: white; border: none; padding: 5px 12px; border-radius: 15px; cursor: pointer; font-size: 12px; transition: 0.3s; }}
    ...
    for r in res:
        try:
            ...
            if m:
                ...
                total_out += m
                if m <= 15000: gastos_hormiga += m
        except: pass

    msg_out = f"💸 Total Gastado: <b>${total_out:,.0f}</b><br>🐜 Gastos Hormiga: <b>${gastos_hormiga:,.0f}</b>"
...
try:
    ...
    crear_app_interactiva("tareas.html", "Progreso Tareas", ["Completadas", "Pendientes"], [hechas, pendientes], 
                          ['#2ea043', '#555'], msg_t)
except Exception as e: 
    print("Error en Tareas (Revisa DB_TAREAS):", e)
...
try:
    ...
    habs = {}
    # Nombres exactos de las columnas de Checkbox en tu tabla de Hábitos
    nombres_habitos = ["Meditacion", "Leer", "Ejercicio", "Agua 2LT", "Agradecer"] 
    
    for hab in nombres_habitos:
        habs[hab] = 0 # Inicializar contador

    for r in res_h:
        for hab in nombres_habitos:
            # Busca si la propiedad existe y si está marcada como True
            prop = r["properties"].get(hab)
            if prop and prop["type"] == "checkbox" and prop["checkbox"] == True:
                habs[hab] += 1
    
    msg_h = "¡Sigue sumando días! 🔥"
    crear_app_interactiva("habitos.html", "Mis Hábitos", list(habs.keys()), list(habs.values()), 
                          ['#36a2eb', '#ffce56', '#4bc0c0', '#ff6384', '#9966ff'], msg_h)

except Exception as e: 
    print("Error en Hábitos (Revisa nombres de columnas):", e)
